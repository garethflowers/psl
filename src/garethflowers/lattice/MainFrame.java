 ...
 * </p>
 * <p>
 * Description: Main frame for all views
 * </p>
 * <p>
 * Copyright: Gareth Flowers Copyright (c) 2005
 * </p>
 * <p>
...
class MainFrame extends JFrame implements ActionListener, ListSelectionListener,
    ...
    private final ArrayList list_array_nodes = new ArrayList(0);
    private final ArrayList list_array_results = new ArrayList(0);
    ...
    private void create_views() {
        ...
        this.array_nodes = (int[][]) this.list_array_nodes.get(0);
    ...
    private void get_lattice_results(final int num) {
        double temp_size = 0;
        this.array_results = (double[][]) this.list_array_results.get(num);
        this.list_clusters.removeAll();
    ...
    private void hide_gui() {
        this.wait_dialog.setLocation((this.getWidth() / 2)
                - (this.wait_dialog.getWidth() / 2), (this.getHeight() / 2)
                - (this.wait_dialog.getHeight() / 2));
    ...
    public void itemStateChanged(ListSelectionEvent itemevent) {
        ...
        if (itemevent.getSource() == this.list_lattices) {
            ...
            this.array_nodes = (int[][]) this.list_array_nodes.get(index);
    ...
    private void load_file() {
        ...
        if (filedialog.getFile() != null) {
            try {
                ...
                final FileReader filereader = new FileReader(filedialog.getFile());
                final BufferedReader bufferedreader = new BufferedReader(
                        filereader);
                final ArrayList aList = new ArrayList(0);
    ...
    private void output() {
        ...
        for (int i = 0; i < this.lattice_numbers; i++) {
            int perc_cluster = 0;
            this.array_results = (double[][]) this.list_array_results.get(i);
    ...
    private void save_file() {
        ...
        if (filedialog.getFile() != null) {
            try {
                ...
                for (int i = this.lattice_numbers; --i >= 0;) {
                    this.array_nodes = (int[][]) this.list_array_nodes.get(i);
                    for (int j = this.array_nodes.length; --j >= 0;) {
